    ...
    let promise = new Promise(function(resolve, reject) {
        ...
        googleSpreadsheetEditor.load({
            ...
            spreadsheet.receive(function(err, rows, info) {
                ...
                let existingDataMaxColumns = Number(_.chain(rows).toArray().flatMap(_.keys).max().value());
                logger.info("Determined existing data column count", { existingDataMaxColumns: existingDataMaxColumns});
                ...
                _.keys(rows).forEach(function(k){
                    ...
                    if (dataHasHeader && k == '1') {
                        ...
                        var currentRowMaxColumnNumber = Number(_.max(_.keys(rows[k])));
                        //add empty cells to clear out previous data in row that had more columns
                        for(let columnNumber = currentRowMaxColumnNumber + 1; columnNumber <= existingDataMaxColumns; columnNumber++){
