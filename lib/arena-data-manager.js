...
exports.getContactQueueData = function(active, lastSundayDateFormatted, items){
    let unmergedContactQueue = [];
    items.forEach(function(i){
        let currentList = getContactQueueList(active, i.reason, i.filter);
...
exports.getRosterData = function(active, filter) {
    let members = _.filter(active, { 'isMember': true });
...
exports.getVisitorData = function(active, filter) {
    let visitors = _.filter(active, { 'isMember': false });
...
exports.getEmailLists = function(active){
    let padColumnsCount = 9;
    let activeWithEmail = _.filter(active, function(d){ return d.email });
