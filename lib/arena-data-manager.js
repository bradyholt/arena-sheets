...
exports.readData = function(dataPath, classId, lastSundayDate) {
    ...
    if (attendanceData && attendanceData.length){
        ...
        let firstDateWithDataEndColumnIndex = datesEndColumnIndex;
        let dates = [];
        ...
        for (let i = datesEndColumnIndex; i >= datesStartColumnIndex; i--) {
            ...
            if (isDataForDate) {
                ...
                for(let x = i; i >= datesStartColumnIndex; i--){
        ...
        logger.info("Attendance dates loaded", {class_id: classId, datesStartColumnIndex: datesStartColumnIndex, firstDateWithDataEndColumnIndex: firstDateWithDataEndColumnIndex, dates: dates});
...
exports.getFormattedInactive = function(inactiveRoster) {
    ...
    let sorted = _.sortByOrder(inactiveRoster, [
        function(d) {
            //inactive date
            let date = new Date(0);
    ...
    let formatted = _.map(sorted, function(d){
        let mapped = [
            ...
            d.isActiveMIA ? "MIA" : "Marked Inactive",
            d.lastName || "",
            d.firstName || "",
            d.gender || "",
            d.dob || "",
